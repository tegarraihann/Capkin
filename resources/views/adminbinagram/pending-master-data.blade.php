        ...
        <div class="relative overflow-x-auto shadow-md sm:rounded-lg p-4 bg-white mt-3">
            <div class="w-full flex justify-between">
                ...
                <div class="bg-white">
                    ...
                    <form action="{{ route('filter-data-bidang-ab') }}" method="GET"
                        class="flex items-center text-gray-900 border border-gray-300 rounded-md w-60 bg-gray-50 focus:ring-blue-500 focus:border-blue-500 overflow-hidden">
                        <select name="filter_bidang" id="filter-bidang"
                            class="block py-2 px-4 outline-none text-sm w-full">
                            <option value="">Semua Bidang</option>
                            @foreach ($bidangList as $bidang)
                                @if ($bidang->nama_bidang != 'Pimpinan')
                                <option value="{{ $bidang->id }}" {{ request('filter_bidang') == $bidang->id ? 'selected' : '' }}>
                                    {{ $bidang->nama_bidang }}
                                </option>
                                @endif
                            @endforeach
                        ...
                        <button type="submit"
                            class="px-4 py-2 bg-gradient-to-r from-blue-500 to-blue-400 text-white rounded-r-md hover:bg-blue-600 transition-all hover:to-blue-500 hover:from-blue-400 duration-700">Filter</button>
                    </form>
                </div>
            </div>
